...
//!
//! The *process* underneath the service is [`process_control_plane`]'s subject —
//! how a pass of the loop divides its attention, and what it does when the
//! peer-control-plane record cannot be made durable. This file stays with what
//! the lock does across real process boundaries.
//!
...
// The orchestration harness is shared with `process_control_plane`, and each
// suite reaches a different part of it: the control-plane failure seams and the
// checkpoint paths belong to that one, and this suite never spawns a faulted
// replica or reads a refusal.
