...
//! parser generator. This is the production acceptance fixture's deliberately
//! narrow client surface, not a general service protocol. Peer traffic uses the
//! separately framed and mutually authenticated TLS link.
//!
//! # Requests
//!
//! ```text
...
//! lease-read claim.
//!
//! `LOCAL` is a weaker read through the transport driver's ordinary read
//! surface. It answers with no barrier, no freshness claim, and no read proof,
//! which is why it is a separate verb rather than a parameter: it exists so an
//! operator, or a test, can ask a rejoining replica what *it* holds, and nothing
...
//! replicated state machine. This fixture authenticates peer replicas, not
//! clients; an embedding service still owns client authentication and the
//! expiration-driver role. `CONTRACT.md` records that boundary explicitly.
//!
//! # Responses
//!
//! ```text
...
use rafter::{LogIndex, NodeId, PromotionBarrier};
...
pub enum Request {
    ...
    Local(ResourceName),
    /// Return the complete machine-readable operations snapshot.
    Observe,
    /// Execute one deployment-planned membership input.
...
pub fn parse_request(line: &str) -> Result<Request, RequestError> {
    ...
    match tokens.first().copied() {
        ...
        Some("OBSERVE") => Ok(Request::Observe),
        Some("SHUTDOWN") => Ok(Request::Shutdown),
        Some("ADD_LEARNER") => Ok(Request::Membership(MembershipChange::AddLearner {
            node_id: NodeId(parse_u64(tokens.get(1), "learner node id")?),
            info: NodeInfo::default(),
        })),
        Some("PROMOTE_LEARNER") => {
            let node_id = NodeId(parse_u64(tokens.get(1), "learner node id")?);
            let required = LogIndex(parse_u64(tokens.get(2), "required match index")?);
            Ok(Request::Membership(MembershipChange::PromoteLearner {
                node_id,
                barrier: PromotionBarrier::new(node_id, required),
            }))
        }
        Some("REMOVE_NODE") => Ok(Request::Membership(MembershipChange::RemoveNode {
            node_id: NodeId(parse_u64(tokens.get(1), "node id")?),
        })),
        Some("LEAVE_JOINT") => Ok(Request::Membership(MembershipChange::LeaveJoint)),
        Some("OPEN_SESSION") => {
