...
fn lease_probe_completion_is_bound_to_exact_client_and_message() {
    ...
    assert_eq!(probe_completion_count(history, "c1", 11, 11), Ok(1));
    assert_eq!(probe_completion_count(history, "c2", 11, 11), Ok(0));
    assert_eq!(probe_completion_count(history, "c1", 12, 11), Ok(0));
    assert_eq!(probe_completion_count("", "c1", 11, 11), Ok(0));
    let timeout = history
        .replace(":temporarily-unavailable", ":timeout")
        .replace("code=11", "code=0");
    assert_eq!(probe_completion_count(&timeout, "c1", 11, 0), Ok(1));
}

#[test]
fn lease_probe_history_rejects_incomplete_or_mismatched_operations() {
    assert!(probe_completion_count(COMPLETION, "c1", 11, 11).is_err());
...
fn lease_probe_history_rejects_identity_drift_and_resource_overflow() {
    ...
    assert!(probe_completion_count(&exact_pair, "c1", 11, 11).is_err());
    let missing_value = format!(
        "{{:index 1 :type :invoke :process 0 :f :read :value nil}}\n{}",
        COMPLETION.replace(" :value nil", "")
    );
    assert!(probe_completion_count(&missing_value, "c1", 11, 11).is_err());
    let oversized = "x".repeat(MAX_LINE_BYTES + 1);
    assert!(probe_completion_count(&oversized, "c1", 11, 11)
