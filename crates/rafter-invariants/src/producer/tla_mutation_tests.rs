...
use crate::producer::tla_output::{parse, render_detector_config};
...
fn true_mutation_of_real_predicate_cannot_qualify() {
    ...
    let result = run_tlc_mutation(&root, "true-predicate", &mutated, &detector);
...
fn non_violating_fixture_cannot_qualify() {
    ...
    let result = run_tlc_mutation(&root, "non-violating-fixture", &raft, &non_violating);
    let summary = parse(&result.stdout).expect("parse TLC mutation output");
    assert!(result.status.success());
    assert!(!detector_qualified(
        result.status.code(),
...
fn run_tlc_mutation(root: &Path, name: &str, raft: &str, detector: &str) -> std::process::Output {
    let directory = root
    ...
    fs::write(
        ...
        render_detector_config(&template, "ElectionSafety").expect("render detector config"),
    )
