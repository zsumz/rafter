//! Producer-owned TLA+ mutation-suite qualification scenarios.
...
#[test]
fn producer_mutation_policy_requires_exact_complete_inventory() {
...
#[test]
fn producer_mutation_policy_requires_successful_process_completion() {
    let complete = transcript(REQUIRED_MUTATION_TESTS.iter().copied());
...
#[test]
fn producer_mutation_policy_rejects_noncanonical_cargo_counts() {
    let complete = transcript(REQUIRED_MUTATION_TESTS.iter().copied());
