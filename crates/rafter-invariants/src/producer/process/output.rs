...
fn finalize_process_output(
    ...
    let stdout = retained_result(
        crate::execution::filesystem::read_file(stdout_path),
        stdout_path,
        stderr_path,
        Some(resource_path),
    )?;
    let stderr = retained_result(
        crate::execution::filesystem::read_file(stderr_path),
        stdout_path,
        stderr_path,
        Some(resource_path),
    )?;
    let resource_telemetry = retained_result(
        crate::execution::filesystem::read_file(resource_path),
        stdout_path,
        stderr_path,
        Some(resource_path),
    )?;
    ...
    crate::execution::filesystem::remove_file(stdout_path)?;
    crate::execution::filesystem::remove_file(stderr_path)?;
    crate::execution::filesystem::remove_file(resource_path)?;
    crate::execution::filesystem::remove_file(process_group_path)?;
    crate::execution::filesystem::remove_file(reservation_path)?;
    Ok(ProcessOutput {
        invocation,
