...
use crate::{
    evidence::{ExecutableReceipt, LauncherReceipt},
...
pub(super) const BASH_RUNTIME: &str = "bash";
...
impl BoundProcessRuntime {
    ...
    pub(super) fn receipts(&self) -> BTreeMap<String, ExecutableReceipt> {
        [
            (PERL_RUNTIME, self.perl.receipt()),
            (TIME_RUNTIME, self.time.receipt()),
            (PS_RUNTIME, self.ps.receipt()),
        ]
        .into_iter()
    ...
    pub(super) fn launcher_receipts(
        &self,
...
impl BoundExecutable {
    ...
    pub(super) fn receipt(&self) -> ExecutableReceipt {
        ExecutableReceipt {
            program: self.external_path.to_string_lossy().into_owned(),
...
pub(crate) fn capture_runtime_receipts(
    ...
    let mut receipts = BoundProcessRuntime::bind()?.receipts();
    if include_bash {
        receipts.insert(
            BASH_RUNTIME.to_owned(),
            BoundExecutable::bind_program(BASH_RUNTIME, environment)?.receipt(),
        );
    }
    Ok(receipts)
}

fn launcher(role: &str, runtime: &str, executable: &BoundExecutable) -> LauncherReceipt {
    LauncherReceipt {
        role: role.to_owned(),
        runtime: runtime.to_owned(),
        executable: executable.receipt(),
    }
}
