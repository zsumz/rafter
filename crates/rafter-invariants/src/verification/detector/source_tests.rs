//! Detector source-analysis unit and regression scenarios.
...
#[path = "source_tests/identity_and_resolution.rs"]
mod identity_and_resolution;

macro_rules! identity_and_resolution_tests {
    ($($name:ident),+ $(,)?) => {
        $(
            #[test]
            fn $name() {
                identity_and_resolution::$name();
            }
        )+
    };
}

identity_and_resolution_tests!(
    analyzed_fixture_source_must_own_the_exact_executed_test_identity,
    conditionally_compiled_detector_fixtures_are_rejected,
    auxiliary_rejection_cannot_qualify_a_registered_recorder,
    supplied_source_must_match_its_bound_path,
    target_analysis_cache_invalidates_when_bound_source_changes,
    qualified_calls_are_not_rebound_to_same_leaf_local_helpers,
    exact_inline_fixture_identity_ignores_same_leaf_decoys,
    inline_detector_uses_its_exact_compiler_identity,
    detector_declaration_must_belong_to_its_registered_source_path,
    imported_detector_identity_ignores_same_leaf_module_decoys,
    module_value_bindings_cannot_shadow_the_registered_detector,
    shadowing_and_same_leaf_declarations_are_rejected,
    explicit_external_import_takes_precedence_over_detector_glob,
    a_forged_helper_is_rejected_even_when_a_real_invocation_is_found_first,
);

#[path = "source_tests/call_flow.rs"]
mod call_flow;

#[path = "source_tests/registry.rs"]
mod registry;
