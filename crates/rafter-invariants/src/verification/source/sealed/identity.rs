//! Platform file identity used to detect relinked or replaced sealed paths.
//!
//! A sealed path is bound to the device and inode (or volume and file index)
//! it had at capture, so a later revalidation notices a swap that leaves the
//! bytes and the path spelling unchanged.
...
impl FileIdentity {
    #[cfg(unix)]
    pub(super) fn capture(path: &Path, directory: bool, context: &str) -> Result<Self, String> {
    ...
    pub(super) fn capture(path: &Path, _directory: bool, context: &str) -> Result<Self, String> {
        Err(format!(
