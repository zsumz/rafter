//! Assembly of independently verified raw reports into immutable evidence.
...
use super::{
    ...
    raw::{verify_present_reports, verify_run},
};
use crate::{
    contract::{
...
pub(crate) fn derive_verified_liveness_binding(
    profile: &str,
...
pub(crate) fn verify_present_liveness_reports(
    profile: &str,
    identity: &SimulatorIdentity,
    available_contracts: &[SimulatorLivenessContract],
    events: &BTreeMap<String, Vec<Value>>,
    ...
    for check_id in &identity.checks {
        let execution = expected_execution_contract(profile, check_id)
            .map_err(|message| malformed(format!("liveness run `{check_id}`: {message}")))?;
        let expected_reports = expected_report_contracts(available_contracts, &execution)?;
        for event in events.get(check_id).map(Vec::as_slice).unwrap_or_default() {
            verify_present_reports(profile, check_id, &execution, &expected_reports, event)?;
        }
    }
    Ok(())
}
