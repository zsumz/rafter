//! The persist-before-output runtime trait itself.
//!
//! This module owns the ordering obligation an implementation accepts and the
//! local state it must be able to answer for. It names no medium, no storage
//! layout, and no execution model; choosing those is the implementor's job.
