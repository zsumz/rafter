...
/// The transport's peer set is the link layer's copy of who may speak. A driver
/// that published only on change left it undefined for the whole first
/// incarnation — which is the state a group whose membership never changes
/// stays in forever.
#[test]
fn membership_is_published_to_the_transport_at_construction() {
    let (_driver, transport) = driver_for(1, &[2, 3]);
    ...
    assert_eq!(
        ...
        "the group's membership reached the link layer, without the local node"
    );
}
