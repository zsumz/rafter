...
pub struct TransportDriverOptions {
    ...
    ///
    /// **This is not a cap on the queue, and could not be.** A fence obligation
    /// comes from a committed fact, and a committed fact is not a request: there
    /// is nothing to refuse and nowhere to push back. Discarding an obligation on
    /// overflow would be exactly the forgotten fence the peer control plane
    /// exists to prevent, with a capacity limit attached as an excuse. So the
    /// bound decides something else — when a driver whose link layer has stopped
    /// accepting admission controls should stop accepting client work. Past it
    /// the driver keeps stepping, keeps flushing, and refuses new writes and
    /// reads with [`DriverServiceState::FenceBacklog`], which ends by itself as
    /// soon as the backlog is back within the bound.
    pub max_pending_fences: usize,
}
...
impl TransportDriverOptions {
    ...
    /// Sets [`TransportDriverOptions::max_pending_fences`].
    #[must_use]
    pub const fn with_max_pending_fences(mut self, max_pending_fences: usize) -> Self {
        self.max_pending_fences = max_pending_fences;
        self
    }
    ...
    /// anything, discovered at the first request. A driver that may hold no
    /// fence obligation is the same shape one configuration change later — the
    /// first committed removal would put it permanently past its bound — so both
    /// are answered here rather than at the change.
    pub(super) fn validate(self) -> Result<Self, ManagedDriverError> {
        ...
        if self.max_pending_fences == 0 {
            return Err(ManagedDriverError::InvalidOptions {
                ...
                reason: "a driver that may hold no fence obligation is degraded \
                         by its first committed removal",
            });
        }
        Ok(self)
    }
