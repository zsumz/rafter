...
fn peer_and_global_capacity_release_exactly_on_drain() {
    ...
    queue
        .try_push(peer_a.clone(), 80, envelope(&peer_a, NodeId(1)))
        .expect("first peer A frame");
    assert_eq!(
        queue.try_push(peer_a.clone(), 1, envelope(&peer_a, NodeId(1))),
        Err(InboundQueueError::Full(InboundQueueFull::Peer))
    );
    queue
        .try_push(peer_b.clone(), 80, envelope(&peer_b, NodeId(2)))
        .expect("peer B frame within global limit");
    assert_eq!(
        queue.try_push(peer_c.clone(), 1, envelope(&peer_c, NodeId(3))),
