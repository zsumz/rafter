//! Connection-lifetime scratch reservations against the global receive budget.
//!
//! A full-reserve canonicalization must stay charged for as long as its scratch
//! is retained, and idle receivers' reservations must never exceed the global
//! budget, so retained scratch can never outlive the accounting that bounds it.
...
use super::*;
use crate::{
    ConnectionSequence, PeerFrame, ReceiveMemoryLimits, WireLimits, MIN_SAFE_DECODE_AMPLIFICATION,
    PEER_FRAME_FIXED_BODY_BYTES, PEER_FRAME_LENGTH_PREFIX_BYTES,
};
...
fn full_reserve_canonicalization_stays_charged_while_scratch_is_retained() {
    ...
    let limits = ReceiveMemoryLimits::new(maximum.saturating_mul(4), MIN_SAFE_DECODE_AMPLIFICATION)
...
fn idle_receiver_scratch_reservations_cannot_exceed_the_global_budget() {
    ...
    let codec = codec(maximum);
    let probe =
        PeerFrameScratch::try_with_group_id_capacity(maximum).expect("capacity probe allocation");
