//! Authenticated peer envelopes at the caller-owned transport boundary.
//!
//! An envelope must carry its route and message unchanged, and validation must
//! refuse an unknown group, a wrong recipient, a sender mismatch, and a peer
//! the policy retired or never admitted — each with the reason that names it,
//! so an embedder cannot collapse them into one undifferentiated failure.
