...
fn replay_raft_trace_reaches_expected_final_state() {
    ...
    assert_eq!(report.states().len(), trace.len() + 1);
    assert_eq!(report.states().last(), Some(&expected));
    assert_eq!(report.states()[0].nodes[0].role, Role::Follower);
    assert_eq!(report.states()[1].nodes[0].role, Role::Follower);
    assert_eq!(report.states()[2].nodes[0].role, Role::Candidate);
    assert_eq!(report.states()[2].nodes[0].term, rafter::Term(1));
    assert_eq!(report.states()[3].nodes[1].term, rafter::Term(1));
    assert_eq!(
        trace[2].to_string(),
