...
fn quorum_only_leader_monitor_reports_starved_schedule_bound() {
    ...
    let failure = run_quorum_only_leader_convergence_check(config, 0)
...
fn quorum_only_leader_usability_monitor_reports_exhausted_bound() {
    ...
    let failure = run_quorum_only_leader_usability_check(config, convergence_budget, 0)
...
fn quorum_only_leader_monitor_elects_and_serves_with_minority_unavailable() {
    ...
    assert_eq!(convergence.clause_ids, &["LV-01.a"]);
    assert!(convergence.rounds_used <= convergence.round_limit);
    assert!(convergence.proposal.is_none());
    assert_eq!(usability.clause_ids, &["LV-01.b"]);
    assert!(usability.rounds_used <= usability.round_limit);
    assert!(usability.stable_leader.is_some_and(|evidence| {
