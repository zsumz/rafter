use rafter::{MembershipConfig, MembershipSet, NodeId, PromotionBarrier, Role, Term};

use crate::{Cluster, Envelope};

use super::{
    Action, Bounds, ExplorationState, MessageKind, ProposalId, RestartSnapshotState, SoakAction,
    SoakActionKind, SoakConfig,
};

#[derive(Clone, Debug)]
pub(super) struct EnabledAction {
    pub(super) trace: Action,
    pub(super) operation: Operation,
}

#[derive(Clone, Debug)]
pub(super) struct EnabledSoakAction {
    pub(super) trace: SoakAction,
    pub(super) operation: SoakOperation,
}
...
fn enabled_membership_actions(state: &ExplorationState, bounds: Bounds) -> Vec<EnabledAction> {
...
pub(super) fn enabled_soak_actions(
    state: &ExplorationState,
    config: SoakConfig,
...
pub(super) const fn soak_preferred_kind(step: usize) -> SoakActionKind {
    match step % 18 {
...
fn enabled_membership_operations(state: &ExplorationState) -> Vec<Operation> {
...
fn soak_membership_trace(operation: &Operation) -> SoakAction {
...
fn soak_membership_operation(operation: Operation) -> SoakOperation {
    match operation {
