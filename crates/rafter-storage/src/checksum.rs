...
/// Incremental CRC-32 (IEEE 802.3, the polynomial behind [`crc32`]) over a
/// byte stream fed in arbitrary slices.
///
/// Feeding slices through [`RunningCrc32::update`] and reading
/// [`RunningCrc32::value`] yields exactly `crc32(concatenation)`, so callers
/// can checksum an append-only file without re-reading what is already on
/// disk.
...
impl RunningCrc32 {
    ...
    ///
    /// Calling this repeatedly is equivalent to checksumming the
    /// concatenation of every slice in order.
