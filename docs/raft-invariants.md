...
Assumptions: Sequence numbers and process-local ticks are monotone, and each Maelstrom client issues one synchronous operation at a time. The Maelstrom lease-isolation evidence uses 50 ms ticks, a 20-tick election timeout, a 2-tick heartbeat interval, a 2-tick/100 ms answer deadline, and the resulting 10-tick/500 ms lease window. After expiry it treats each client's first direct read as a warmup, buffers and immediately releases the next direct read from a previously observed client, requires the same node and term through request handling, rejects any second correlated terminal response, and binds the exact client/msg_id bounded code-0 timeout or earlier code-11 unavailable completion to an ordered same-process invoke/terminal pair with matching operation value in retained client history.
